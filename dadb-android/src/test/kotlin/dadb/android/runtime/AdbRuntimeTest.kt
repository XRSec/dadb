...
class AdbRuntimeTest {
    ...
    @Test
    fun discoverPairingService_requiresContextBackedRuntime() {
        val rootDir = Files.createTempDirectory("adb-runtime-test").toFile()
        try {
            val runtime = AdbRuntime(rootDir)

            try {
                runtime.discoverPairingService(timeoutMs = 0)
                fail("Expected discoverPairingService to require a Context-backed runtime")
