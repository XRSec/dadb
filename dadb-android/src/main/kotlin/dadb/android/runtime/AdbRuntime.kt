...
import dadb.android.wireless.AdbMdnsDiscoverer
import dadb.android.wireless.AdbMdnsEndpoint
...
class AdbRuntime(
    ...
    ) : this(defaultStorageRoot(context), options, context.applicationContext)
    ...
    fun discoverPairingService(
        timeoutMs: Long = DEFAULT_MDNS_DISCOVERY_TIMEOUT_MS,
        includeLanDevices: Boolean = true,
    ): AdbMdnsEndpoint? =
        AdbMdnsDiscoverer.discoverPairingService(
            context = requireApplicationContext("discoverPairingService"),
            timeoutMs = timeoutMs,
    ...
    fun discoverConnectService(
        ...
        AdbMdnsDiscoverer.discoverConnectService(
            context = requireApplicationContext("discoverConnectService"),
            timeoutMs = timeoutMs,
            includeLanDevices = includeLanDevices,
        )

    fun createMdnsMonitor(config: AdbMdnsConfig = AdbMdnsConfig()): AdbMdnsMonitor =
        AdbMdnsMonitor(
            context = requireApplicationContext("createMdnsMonitor"),
            config = config,
        )
    ...
    private fun requireApplicationContext(apiName: String): Context =
        checkNotNull(applicationContext) {
            "$apiName requires constructing AdbRuntime with an Android Context"
        }

    companion object {
        private const val USB_RUNTIME_TAG = "DadbUsbRuntime"
        private const val DEFAULT_MDNS_DISCOVERY_TIMEOUT_MS = 12_000L
