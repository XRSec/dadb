...
dadb 2.x distinguishes transport failures from operation outcomes. Transport, authentication,
protocol, timeout, and connection failures throw an `AdbException` subtype. Expected operation
outcomes are returned as `InstallResult`, `UninstallResult`, `SyncResult`, or `RootResult`.
