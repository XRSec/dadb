...
1. Create a new branch for the release `git checkout -b release-vX.Y.Z` (where X.Y.Z is the new
   version)
...
8. `git checkout master && git pull && git checkout -b prepare-X.Y.Z-SNAPSHOT` (where X.Y.Z is the
