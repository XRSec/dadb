...
val buildToolsDir =
    file("$androidSdkDir/build-tools")
        .listFiles()
        ?.filter { it.isDirectory && File(it, "d8").isFile }
        ?.maxByOrNull { it.name }
        ?: error("Android build-tools not found under $androidSdkDir/build-tools")

val androidJar = file("$androidSdkDir/platforms/android-37.0/android.jar")
...
val dexHelperClasses = tasks.register<Exec>("dexHelperClasses") {
