...
internal abstract class BaseForwarder(
    ...
    private fun handleForwarding() {
        ...
        while (!Thread.currentThread().isInterrupted) {
            ...
            try {
                // The remote channels must be opened in the same order as local accept().
                // Protocols such as scrcpy assign video/audio/control roles by connection order.
                // Opening the ADB channels inside the worker pool races those roles even when the
                // local clients connect sequentially.
                val remoteChannel = remoteChannelFactory(remoteDestination)
                clientExecutor?.execute {
                    try {
                        handleClient(client, remoteChannel)
                    } catch (t: Throwable) {
                ...
                log { "forwarder channel open failed for $endpointDescription -> $remoteDestination: ${t.message}" }
                runCatching { client.close() }
            }
        }
    }

    private fun handleClient(client: ForwardingClient, remoteChannel: ForwardingDuplex) {
