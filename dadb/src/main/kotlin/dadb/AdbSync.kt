...
class AdbSyncStream(
    ...
    private fun readSendStatus() {
        ...
        when (packet.id) {
            ...
            else -> throw AdbProtocolException("Unexpected sync packet id: ${packet.id}")
        }
    }
            ...
            when (packet.id) {
                DATA -> {
                    ...
                    if (chunkSize < 0 || chunkSize > SYNC_DATA_MAX) {
                        throw AdbProtocolException("Sync DATA chunk too large: $chunkSize > $SYNC_DATA_MAX")
                    }
    ...
    private fun readStatV2(path: String, requestId: String, apiName: String): AdbSyncStatV2 {
        ...
        if (responseId != STAT_V2 && responseId != LSTAT_V2) {
            throw AdbProtocolException("Unexpected sync stat id: $responseId")
        }
    ...
    private fun readLstatV1(path: String): AdbSyncStat {
        ...
        val responseId = readSyncId()
        if (responseId != STAT) {
            throw AdbProtocolException("Unexpected sync lstat id: $responseId")
        }
    ...
    private fun readFailException(length: Int? = null): IOException {
        ...
        if (messageLength < 0) {
            return AdbProtocolException("Sync failed with invalid message length: $messageLength")
        }
        val message = stream.source.readString(messageLength.toLong(), StandardCharsets.UTF_8)
        return AdbSyncFailException(message)
    }
