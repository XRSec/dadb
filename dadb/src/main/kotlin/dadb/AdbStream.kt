...
internal class AdbStreamImpl internal constructor(
    ...
    override val sink = object : Sink {
        ...
        override fun flush() {
            ...
            if (!delayedAckEnabled && nextMessage(Constants.CMD_OKAY) == null) {
                throw AdbConnectionClosedException("ADB stream closed before OKAY for localId: ${localId.toString(16)}")
            }
        }
    ...
    private fun awaitAckBytes(): Int {
        val message = nextMessage(Constants.CMD_OKAY)
            ?: throw AdbConnectionClosedException("ADB stream closed before delayed ACK for localId: ${localId.toString(16)}")
    ...
    private fun nextMessage(command: Int): AdbMessage? {
        return try {
            ...
            throw if (e is SocketTimeoutException) {
                AdbTimeoutException("Read timed out on stream $localId; device unresponsive", e)
            } else {
                AdbConnectionClosedException("Connection lost while reading stream $localId", e)
            }
        }
    }
