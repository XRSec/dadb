...
internal class DadbResultTest {
    ...
    fun syncRecvFailThrowsSyncFailException() {
        val stream = ResultFakeAdbStream(syncFailBuffer("No such file or directory"))
    ...
    fun syncRecvUnexpectedPacketThrowsProtocolException() {
        ...
        val sync = AdbSyncStream(ResultFakeAdbStream(bogus))
        assertFailsWith<AdbProtocolException> { sync.recv(Buffer(), "/somewhere") }
    }

    @Test
    fun pullMissingFileReturnsSyncFailure() {
        val dadb = FakeDadb { ResultFakeAdbStream(syncFailBuffer("No such file or directory")) }
    ...
    fun pushSuccessReturnsSyncSuccess() {
        val dadb = FakeDadb { ResultFakeAdbStream(syncOkayBuffer()) }
    ...
    fun uninstallNonZeroExitReturnsFailureWithExitCode() {
        val dadb = FakeDadb {
            ResultFakeAdbStream(shellV2Buffer(stdout = "Failure [DELETE_FAILED_INTERNAL_ERROR]\n", exitCode = 1))
        }
    ...
    fun uninstallZeroExitReturnsSuccess() {
        val dadb = FakeDadb { ResultFakeAdbStream(shellV2Buffer(stdout = "Success\n", exitCode = 0)) }
    ...
    fun installCmdPathFailureReturnsInstallFailure() {
        val dadb = FakeDadb(features = setOf("cmd")) {
            ResultFakeAdbStream(Buffer().also { it.writeUtf8("Failure [INSTALL_FAILED_INVALID_APK]") })
        }
    ...
    fun installCmdPathSuccessReturnsSuccess() {
        val dadb = FakeDadb(features = setOf("cmd")) {
            ResultFakeAdbStream(Buffer().also { it.writeUtf8("Success\n") })
        }
        val result = dadb.install(Buffer().also { it.writeUtf8("apk") }, 3L)
    ...
    fun rootFailureReturnsRootFailure() {
        val dadb = FakeDadb {
            ResultFakeAdbStream(Buffer().also { it.writeUtf8("adbd cannot run as root in production builds\n") })
        }
